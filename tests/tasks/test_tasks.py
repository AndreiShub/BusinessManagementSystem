...
async def test_get_task(client: AsyncClient, db_session: AsyncSession, auth_headers):
    ...
    response = await client.get(f"/api/v1/teams/{team.id}/tasks/{task.id}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_create_task(client: AsyncClient, db_session: AsyncSession, auth_headers):
    ...
    response = await client.post(f"/api/v1/teams/{team.id}/tasks", json=payload, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_update_task_status_only(client: AsyncClient, db_session: AsyncSession, auth_headers):
    user = await create_user(db_session)
    team = await create_team(db_session, creator=user)
    task = await create_task(db_session, team_id=team.id, creator_id=user.id)

    payload = {"status": "done"}
    response = await client.patch(f"/api/v1/teams/{team.id}/tasks/{task.id}", json=payload, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_delete_task(client: AsyncClient, db_session: AsyncSession, auth_headers):
    ...
    response = await client.delete(f"/api/v1/teams/{team.id}/tasks/{task.id}", headers=auth_headers)
...
async def test_rate_task(client: AsyncClient, db_session: AsyncSession, auth_headers):
    ...
    payload = {"score": 5}
    response = await client.post(f"/api/v1/teams/{team.id}/tasks/{task.id}/rating", json=payload, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_add_comment(client: AsyncClient, db_session: AsyncSession, auth_headers):
    ...
    response = await client.post(f"/api/v1/teams/{team.id}/tasks/{task.id}/comments", json=payload, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
