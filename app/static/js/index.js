...
function formatDateUTC(year, month, day) {
    // Форматируем строго в UTC: YYYY-MM-DD
...
async function fetchEvents(year, month) {
    try {
        const response = await fetch(
            `/api/v1/events?year=${year}&month=${month}`,
            {
                headers: {
...
function showEventsForSelectedDay() {
    ...
    dayEvents.forEach(event => {
        ...
        if (event.event_type === 'meeting') {
            ...
            if (event.status === 'completed') statusColor = '#28a745';
            else if (event.status === 'cancelled') statusColor = '#dc3545';
            else if (event.status === 'in_progress') statusColor = '#ffc107';
        ...
        li.addEventListener('click', () => showEventDetails(event.id));
